...
- Only selected TV Time history files are read: supported legacy CSV files and current `tvtime-series`/`tvtime-movies` JSON files.
- HTML summaries and unrelated/sensitive files are ignored and never sent to the server.
...
The MVP supports TV Time GDPR exports into SagaLog and BetaSeries watched episodes, show library rows, watched movies, and movie list rows. Current TV Time JSON exports preserve TVDB IDs for series, episodes, and movies, plus IMDb IDs for movies. Episode imports use exact episode IDs with bulk backfilling disabled. Movie watch rows become `seen`; movie list/follow rows become `to see`. Ratings, emotion votes, comments, friends, devices, login records, IP records, and tokens are intentionally out of scope.
